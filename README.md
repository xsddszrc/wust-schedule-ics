...
`git pull` 用于拉取脚本的最新更新（验证码识别算法、教务系统适配等变动会持续推送）。
...
> 注意：`&` 启动的后台进程在服务器重启后会消失。如需持久运行，建议用 systemd 或 supervisor 管理，或直接把 `--output` 写到已有 Web 服务的静态目录下（方式 A）。
...
安装 [wust-schedule-ics.user.js](https://xsddszrc.github.io/wust-schedule-ics/wust-schedule-ics.user.js) → 登录教务 → 打开[课表页面](https://bkjx.wust.edu.cn/jsxsd/xskb/xskb_list.do) → 点击"📅 导出ICS"。
...
| `.venv/bin/python generate_ics.py --login --no-headless` | 同上，显示浏览器窗口（调试用） |
...
ddddocr 容易把数字误识别（如 `0→o`、`1→l`、`9→g`），对运算符的识别也有偏差（`×→x/X`、`÷→?`）。脚本内置了两轮修正：先修正数字字符，再将 `x`/`X`/`×` 归一化为 `*`（乘号）、`÷` 归一化为 `/`（除号），然后根据实际运算符计算答案。单次识别准确率约 90%，配合最多 6 次重试，总体通过率接近 100%。
...
| `wust_ics.log` | 运行日志（每次运行追加） |

## 🐛 反馈问题

脚本每次运行都会在项目目录生成 `wust_ics.log`，终端输出的所有内容同步写入该文件。遇到问题请附上 `wust_ics.log` 文件内容，到 [GitHub Issues](https://github.com/xsddszrc/wust-schedule-ics/issues) 提交反馈。
